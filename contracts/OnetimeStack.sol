...
contract OnetimeStack {
  ...
  constructor(
    ...
    long = _long;
    minFullFundingWei = calcMinFullFunding();
    fullyFunded = false;
    internalBalance = 0;
    payoff = 0;
    payoffSet = false;
  ...
  function calcMinFullFunding() private view returns (uint256) {

    uint256 refPrice = getReferencePrice();

    if (!long) { return strikeWei; }

    if (refPrice <= strikeWei) {
      return (refPrice / 2);
    }

    return (3 * (refPrice - strikeWei) / 2);
  }

  function calcPayoff() private view returns (uint256) {

    uint256 refPrice = getReferencePrice();

    if (long) {

      if (refPrice <= strikeWei) { return 0; }

      return (refPrice - strikeWei);
    }

    if (refPrice >= strikeWei) { return 0; }

    return (strikeWei - refPrice);
  }
  ...
  function ping() external {
    ...
    uint256 funding = calcMinFullFunding();

    if (minFullFundingWei < funding) {
      minFullFundingWei = funding;
    }
    ...
    if (block.timestamp >= maturityUnix) {
      ...
      if (payoffSet && hasPaymentsOutstanding()) {
        ...
        if (!inTheMoney) {
          ...
          payoff = calcPayoff();
          minFullFundingWei = payoff;

          if (internalBalance >= minFullFundingWei) { fullyFunded = true; }
